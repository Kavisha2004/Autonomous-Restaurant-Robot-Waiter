 ...
 * and live Customer Feedback & Rating Monitoring (Sri Lankan Rupees Rs. / LKR).
 */
...
document.addEventListener("DOMContentLoaded", () => {
        ...
        <div class="table-status-card ${isOccupied ? 'occupied' : ''} ${t.status === 'delivering' ? 'delivering' : ''}" id="card-table-${t.id}">
          ...
          <div class="table-card-details">
            ${isOccupied && activeOrder ? `
              <div style="font-weight: 700; color: var(--wood-dark); margin-bottom: 0.2rem;">${activeOrder.customerName}</div>
          ...
          <div class="order-card-body">
            ...
            ${order.specialNote && order.specialNote !== "No special requests" && order.specialNote !== "None" ? `
              <div class="special-note-box">
                ...
                <div>
                  <strong>Special Note / Allergy:</strong> ${order.specialNote}
                </div>
              </div>
            ` : ""}
      ...
      return `
        <div class="feedback-item-card">
          <div class="feedback-item-top">
            <div>
              <strong style="color: var(--wood-dark); font-size: 1rem; display: block;">${fb.customerName}</strong>
          ...
          <div style="display: flex; flex-direction: column; gap: 0.25rem; margin-bottom: 0.75rem; background: var(--bg-main); padding: 0.6rem 0.8rem; border-radius: var(--radius-sm); border: 1px solid var(--wood-soft);">
            <div style="font-size: 0.8rem; display: flex; justify-content: space-between;">
              <span style="color: var(--text-muted);">🍱 Food Quality:</span>
              <span class="feedback-stars">${foodStarsStr}</span>
            </div>
            <div style="font-size: 0.8rem; display: flex; justify-content: space-between;">
              <span style="color: var(--text-muted);">🤖 Robot Delivery:</span>
          ...
          ${fb.tags && fb.tags.length > 0 ? `
            <div style="display: flex; flex-wrap: wrap; gap: 0.35rem; margin-bottom: 0.6rem;">
              ${fb.tags.map((t) => `<span class="badge" style="background: var(--wood-soft); color: var(--wood-dark); font-size: 0.7rem; border: 1px solid var(--wood-border);">${t}</span>`).join("")}
            </div>
          ` : ""}
  ...
  // --- 7. Manual Robot Control Action Buttons ---
  ...
  // --- 8. Realtime Subscription & Auto Polling ---
  function renderAll() {
    renderMetrics();
