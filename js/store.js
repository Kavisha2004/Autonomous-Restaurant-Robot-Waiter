...
const Store = (function () {
  ...
  function resetAllData() {
    ...
    if (supabaseClient) {
      // Re-seed default tables and robot in Supabase
      supabaseClient.from("tables_status").upsert(getDefaultTables().map(tableToDb)).then(() => {});
      supabaseClient.from("robot_state").upsert(robotToDb(getDefaultRobot())).then(() => {});
    }
  }
